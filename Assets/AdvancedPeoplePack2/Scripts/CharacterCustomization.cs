...
namespace AdvancedCustomizableSystem
{
        ...
        private Dictionary<string, float> bodyShapeWeight = new Dictionary<string, float>
    {
      ...
      { "Angry", 0 }
        ...
        #region Custom
        public void ChangeTag(Player player, Camera camera)
        {
            if (player.isLocalPlayer)
            {
                if (bakeGroup == null || !bakeGroup.activeSelf)
                {
                    foreach (var part in characterParts)
                    {
                        ...
                        {
                           skinnedMesh.gameObject.layer = LayerMask.NameToLayer("LocalPlayerVisibleOnly");
                        }
                    }
                ...
                camera.cullingMask = GeneralManager.singleton.localPlayer;
            }
            else camera.cullingMask = GeneralManager.singleton.notLocalPlayer;
        }
        #endregion
        ...
        public void BakeCharacter(bool includeBlendShapes = true, bool saveMeshes = false)
        {
            ...
            RecalculateLOD();
            //GeneralManager.singleton.SetChildLayer(bakeGroup.transform, "bake_mesh", 20);
        }
        /// <summary>
