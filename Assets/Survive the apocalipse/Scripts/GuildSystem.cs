...
public static class GuildSystem
{
    ...
    public static void TerminateGuild(string guildName, string requester)
    {
            ...
            guild.CanTerminate(requester))
        {
            ...
            foreach (KeyValuePair<string, Player> players in Player.onlinePlayers)
            {
                Player playerOnline = players.Value;
                
                if(playerOnline.guild.name == guildName)
                {
                    for(int i = 0; i < playerOnline.playerAlliance.guildAlly.Count; i++)
                    {
                        int index = i;
                        playerOnline.playerAlliance.guildAlly.Remove(playerOnline.playerAlliance.guildAlly[index]);
                    }
                }
