...
public partial class Monster : Entity
{
    ...
    [Header("Grab timer")]
    public double monsterGradTimer;
    ...
    protected override void OnDeath()
    {
        ...
        foreach (ItemDropChance itemChance in dropChances)
            if (Random.value <= itemChance.probability)
                inventory.Add(new ItemSlot(new Item(itemChance.item), itemChance.amount));
    ...
    {
        if (entity && entity is Player && entity.isServer)
        {
            if (health > 0)
            {
                if (((Player)entity).playerCar.car)
                {
                    return false;
                }

                if (base.CanAttack(entity) && ((Player)entity).playerMove.sneak && ((Player)entity).playerBlood.currentBlood > 0 && ((Player)entity).state != "CASTING")
                {
                    return false;
                }
                return true;
