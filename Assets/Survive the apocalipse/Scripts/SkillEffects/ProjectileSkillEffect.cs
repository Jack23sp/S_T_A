...
public class ProjectileSkillEffect : SkillEffect
{
    ...
    public int damage = 1; // set by skill
    ...
    {
        if (!((Player)caster).playerOptions.blockSound)
        {
            if (((WeaponItem)((Player)caster).playerItemEquipment.firstWeapon.item.data).shotClip)
            {
                GeneralManager.singleton.shotAudioSource.clip = (((WeaponItem)((Player)caster).playerItemEquipment.firstWeapon.item.data).shotClip);
                GeneralManager.singleton.shotAudioSource.Play();
                if (((Player)caster).isLocalPlayer ||
                    (((Player)caster).target != null && !((Player)caster).isLocalPlayer &&
                    Vector3.Distance(((Player)caster).transform.position, ((Player)caster).target.transform.position) <= 5))
                {
                    if (((WeaponItem)((Player)caster).playerItemEquipment.firstWeapon.item.data).stopMusic)
                    {
                        SoundManager.singleton.Shoot();
                    }
                }
            }
        }
    ...
    public void BulletDrop()
    {
        if (!((Player)caster).playerOptions.blockSound)
        {
            if (((WeaponItem)((Player)caster).playerItemEquipment.firstWeapon.item.data).bullettDrops)
            {
                GeneralManager.singleton.bulletsDropAudioSource.clip = (((WeaponItem)((Player)caster).playerItemEquipment.firstWeapon.item.data).bullettDrops);
                GeneralManager.singleton.bulletsDropAudioSource.Play();
            }
        }
    }
        ...
        if (target != null && caster != null)
        {
            ...
            if (isServer && transform.position == goal)
            {
                if (target.health > 0)
                {
                    ...
                    caster.DealDamageAt(target, damage, stunChance, stunTime);
                }
