...
public class TargetProjectileSkill : DamageSkill
{
    ...
    bool HasRequiredWeaponAndAmmo(Entity caster)
    {
        ...
        int ammoIndex = caster.GetEquippedAmmoIndex();
        if (weaponIndex != -1 && ammoIndex != -1)
        {
            // no ammo required, or has that ammo equipped?
            WeaponItem itemData = (WeaponItem)caster.equipment[weaponIndex].item.data;
            return ((WeaponItem)caster.equipment[weaponIndex].item.data).ammoItems.Contains((AmmoItem)caster.equipment[ammoIndex].item.data);
        }
        return false;
    }
