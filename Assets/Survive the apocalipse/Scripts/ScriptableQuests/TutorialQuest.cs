// a simple location quest example
//
// how to use locations:
// - create an empty GameObject
// - add a CircleCollider2D with IsTrigger enabled
// - set the layer to IgnoreRaycast so that we can still click through it
// - set the tag to QuestLocation so that it's forwarded to the quest system
// - name it the same as the quest
//
// if you need multiple locations for one quest, then create another quest type
// with a list of location names and set the separate bits in 'field0' to keep
// track of what was visited.
using UnityEngine;
using System.Text;
using System.Collections.Generic;

[System.Serializable]
public struct QuestAbility
{
    public ScriptableAbility ability;
    public int level;
    public bool completed;
}

[System.Serializable]
public struct QuestBoost
{
    public ScriptableBoost boosts;
    public bool completed;
}

[System.Serializable]
public struct ZombieKill
{
    public Monster monster;
    public int quantity;
}

[System.Serializable]
public struct UpgradeBuilding
{
    public ScriptableBuilding buildingType;
    public int levelToReach;
    public int itemLevelToReach;
}

[System.Serializable]
public struct UpgradeItems
{
    public ScriptableItem item;
    public int levelToReach;
}


[System.Serializable]
public struct SetSpawnpoint
{
    public int numberOfSpawnpointToCreate;
}

[CreateAssetMenu(menuName="uMMORPG Quest/Tutorial Quest", order=999)]
public class TutorialQuest : ScriptableQuest
{


    // events //////////////////////////////////////////////////////////////////
    public override void OnLocation(Player player, int questIndex, Collider2D location)
    {
        // the location counts if it has exactly the same name as the quest.
        // simple and stupid.
        if (location.name == name)
        {
            Quest quest = player.quests[questIndex];
            quest.progress = 1;
            player.quests[questIndex] = quest;
        }
    }

    // fulfillment /////////////////////////////////////////////////////////////
    public override bool IsFulfilled(Player player, Quest quest)
    {
        return quest.progress == 1;
    }

    // tooltip /////////////////////////////////////////////////////////////////
    public override string ToolTip(Player player, Quest quest)
    {
        // we use a StringBuilder so that addons can modify tooltips later too
        // ('string' itself can't be passed as a mutable object)
        StringBuilder tip = new StringBuilder(base.ToolTip(player, quest));
        tip.Replace("{LOCATIONSTATUS}", quest.progress == 0 ? "Pending" : "Done");
        return tip.ToString();
    }
}
