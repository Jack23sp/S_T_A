...
public partial class Pet : Summonable
{
    ...
    [SyncVar, HideInInspector] public long _experience = 0;
    public long experience
    {
        ...
        set
        {
            ...
            else
            {
                ...
                Debug.Log("Experince :" + _experience);
                Debug.Log("Experince max:" + experienceMax);
                Debug.Log("Level :" + level);
                Debug.Log("Level max :" + maxLevel);
    ...
    public void SyncPetStateInPlayer()
    {
        owner.petItem.item.summonedHealth = health;
        owner.petItem.item.summonedLevel = level;
        owner.petItem.item.summonedExperience = experience;
    }
    ...
    public override void OnStartServer()
    {
        ...
        // if is spawned already death
        if (health == 0) { deathTimeEnd = NetworkTime.time + deathTime;  _state = "DEAD"; }
    ...
    public void RecalculateExperience(long exp)
    {
        while (_experience >= experienceMax && level < maxLevel)
        {
            // subtract current level's required exp, then level up
    ...
    public void ClearOwnerSummoned()
    {
        for (int i = 0; i < owner.inventory.Count; i++)
        {
            if (owner.inventory[i].amount == 0) continue;
            if (owner.inventory[i].item.data is PetItem)
            {
                ItemSlot petItem = owner.inventory[i];
                if (petItem.item.isSummoned)
                {
                    petItem.item.isSummoned = false;
                    owner.inventory[i] = petItem;
                }
            }
        }

        for (int i = 0; i < owner.playerBelt.belt.Count; i++)
        {
            if (owner.playerBelt.belt[i].amount == 0) continue;
            if (owner.playerBelt.belt[i].item.data is PetItem)
            {
                ItemSlot petItem = owner.playerBelt.belt[i];
                if (petItem.item.isSummoned)
                {
                    petItem.item.isSummoned = false;
                    owner.playerBelt.belt[i] = petItem;
                }
            }
        }
    }
    ...
    public override void DealDamageAt(Entity entity, int amount, float stunChance = 0, float stunTime = 0)
    {
        ...
        if (entity is Monster monster)
        {
            ...
            owner.OnDamageDealtToMonster(monster);
        }
        // a player?
        ...
        else if (entity is Player player)
        {
            ...
            owner.OnDamageDealtToPlayer(player);
        }
        // a pet?
        // (see murder code section comments to understand the system)
        else if (entity is Pet pet)
        {
            owner.OnDamageDealtToPet(pet);
        }
    ...
    public void SyncToOwner()
    {
        for (int i = 0; i < owner.inventory.Count; i++)
        {
            ...
            if (owner.inventory[i].item.data is PetItem)
            {
                if (owner.inventory[i].item.data.name == name)
                {
                    if (owner.inventory[i].item.isSummoned == true)
                    {
                        ItemSlot petItem = owner.inventory[i];
                        petItem.item.summonedHealth = health;
                        petItem.item.summonedLevel = level;
                        petItem.item.summonedExperience = experience;
                        owner.inventory[i] = petItem;
                    }
                }
            }
        ...
        for (int i = 0; i < owner.playerBelt.belt.Count; i++)
        {
            ...
            if (owner.playerBelt.belt[i].item.data is PetItem)
            {
                if (owner.playerBelt.belt[i].item.data.name == name)
                {
                    if (owner.playerBelt.belt[i].item.isSummoned == true)
                    {
                        ItemSlot petItem = owner.playerBelt.belt[i];
                        petItem.item.summonedHealth = health;
                        petItem.item.summonedLevel = level;
